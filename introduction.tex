...
%However, to date, the mechanism of the formation of the knots is not clearly understood. Although a number of theories have been developed to explain these bright knots, they are restricted to explain particular sources. In this section I discuss the theories of the formation of bright knots.

%In case of the knots of M81, more than one theory has been employed to interpret their formation.
...
%To my knowledge, this strategy has never been previously attempted in the literature.
...
%\end{equation}
%
%The estimate of the jet velocity using Eqn.~\eqref{eq:db} assumes that the jet and counter jet are equally powerful and and equally fast and are pointing in exactly opposite directions. Therefore, Doppler beaming estimates involve large uncertainties-- i) If the intrinsic brightnesses of the two jets are different. For example, Hydra A has four knots on the southern side compared to two on the northern side leading to an intrinsic difference in brightness asymmetry. ii) If the jets have curved structure, for example, Hydra A jets iii) If the counter jet is not visible, for example, M87. Sometimes the Doppler beaming estimate may be misleading if there is some asymmetry in the structure of the jet and the counter jet \citep{kovalev07}.
%
%In this thesis, I explored the possibility of an alternative theoretical approach to estimate the jet velocity. Here, I showed that in an appropriate case, if the bright knots of a jet can be modelled by reconfinement shocks, the information of bright knots and oscillations of the jet boundary near the core can be used to estimate the velocity of AGN jets.

%In this thesis, I explored the possibility of an alternative theoretical approach to estimate the jet velocity. Here, I showed that how the information of bright knots and the oscillation of the jet boundary near the core can be used to estimate the jet velocity. 
...
\begin{figure*}
\centering
\includegraphics[width=15cm]{hyc.eps}
...
%Specifically, this work introduces the strategy of estimating the jet velocity from the knot spacings and radial oscillations of the inner radio jet simulateneously through hydrodynamic modelling. The results from this procedure 
%confirm the association of bright knots with reconfinement shocks produced when an over pressured jet comes into pressure equilibrium with the ambient medium.

%Here I briefly describe the key contributions of this thesis. 
...
%The location of biconical shocks are shown in panel (b) of Fig.~\ref{hyc}. 
...
From the 3D precessing jet models, matching the simulated jet curvature and the jet to plume transition with the observation, I estimate the precession period $P \approx 1$~Myr and precession angle $\psi = 20^{\circ}$ of the Hydra A jets. 
%%\begin{equation}
